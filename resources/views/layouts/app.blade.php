...
<head>
    ...
    <title>{{ config('app.name', 'HR Assistance') }} - Dashboard</title>
                ...
                @foreach($navItems as $item)
                    <a href="{{ route($item['route']) }}"
                       class="flex items-center px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 group
                              {{ request()->routeIs($item['route']) ? 'bg-purple-500/10 text-purple-400' : 'text-gray-400 hover:text-white hover:bg-[#1C1C1E]' }}">
                        <div class="relative">
            ...
            <header class="sticky top-0 z-10 bg-[#09090B]/80 backdrop-blur-lg border-b border-[#1C1C1E]">
                <div class="flex items-center justify-between px-8 py-4">
                    <div>
                        ...
                        <p class="text-sm text-gray-400">@yield('page-subtitle', 'Overview of your career tools')</p>
                    </div>
                    <div class="flex items-center space-x-4">
                        <!-- Search (optional) -->
                        <div class="hidden md:flex items-center bg-[#1A1A1E] rounded-lg px-3 py-2 border border-[#1C1C1E]">
                            <svg class="w-4 h-4 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
                            </svg>
                            <input type="text" placeholder="Search..."
                                   class="bg-transparent border-none text-white text-sm ml-2 focus:outline-none w-40">
                        </div>
